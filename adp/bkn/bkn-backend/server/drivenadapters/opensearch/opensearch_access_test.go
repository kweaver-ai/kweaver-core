...
func Test_openSearchAccess_PutIndexTemplate(t *testing.T) {
	Convey("test PutIndexTemplate\n", t, func() {
		appSetting := &common.AppSetting{}
		osa, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
			roundTripFunc: func(req *http.Request) (*http.Response, error) {
				So(req.Method, ShouldEqual, "PUT")
				So(req.URL.Path, ShouldContainSubstring, "_index_template")
		...
		Convey("PutIndexTemplate Success \n", func() {
			err := osa.PutIndexTemplate(testCtx, "test-template", templateBody)
			So(err, ShouldBeNil)
		})

		Convey("PutIndexTemplate Failed - marshal error\n", func() {
			// 创建一个无法序列化的对象
			invalidBody := make(chan int)
			err := osa.PutIndexTemplate(testCtx, "test-template", invalidBody)
			So(err, ShouldNotBeNil)
		})

		Convey("PutIndexTemplate Failed - HTTP error\n", func() {
			...
			err := osa2.PutIndexTemplate(testCtx, "test-template", templateBody)
			So(err, ShouldNotBeNil)
		})

		Convey("PutIndexTemplate Failed - response error\n", func() {
			...
			err := osa3.PutIndexTemplate(testCtx, "test-template", templateBody)
			So(err, ShouldNotBeNil)
		})
	})
}
...
func Test_openSearchAccess_BulkInsertData(t *testing.T) {
	Convey("test BulkInsertData\n", t, func() {
		...
		dataList := []any{
			map[string]any{
				interfaces.OBJECT_ID: "doc1",
				"title":              "Document 1",
			},
			map[string]any{
				interfaces.OBJECT_ID: "doc2",
				"title":              "Document 2",
			},
		}
...
func Test_openSearchAccess_GetIndexStats(t *testing.T) {
	Convey("test GetIndexStats\n", t, func() {
		appSetting := &common.AppSetting{}
		osa, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
			roundTripFunc: func(req *http.Request) (*http.Response, error) {
				So(req.Method, ShouldEqual, "GET")
				So(req.URL.Path, ShouldContainSubstring, "test-index")
				So(req.URL.Path, ShouldContainSubstring, "_stats")
				...
				respBytes, _ := json.Marshal(statsResponse)
		...
		Convey("GetIndexStats Success \n", func() {
			stats, err := osa.GetIndexStats(testCtx, "test-index")
			So(err, ShouldBeNil)
			So(stats, ShouldNotBeNil)
			So(stats.DocCount, ShouldEqual, 1000)
			So(stats.StorageSize, ShouldEqual, 1024000)
		})

		Convey("GetIndexStats Failed - HTTP error\n", func() {
			osa2, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
				roundTripFunc: func(req *http.Request) (*http.Response, error) {
					return nil, errors.New("network error")
				},
			...
			stats, err := osa2.GetIndexStats(testCtx, "test-index")
			So(stats, ShouldBeNil)
			So(err, ShouldNotBeNil)
		})

		Convey("GetIndexStats Failed - response error\n", func() {
			osa3, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
				roundTripFunc: func(req *http.Request) (*http.Response, error) {
					return &http.Response{
						StatusCode: 400,
			...
			stats, err := osa3.GetIndexStats(testCtx, "test-index")
			So(stats, ShouldBeNil)
			So(err, ShouldNotBeNil)
		})

		Convey("GetIndexStats Failed - unmarshal error\n", func() {
			osa4, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
				roundTripFunc: func(req *http.Request) (*http.Response, error) {
					return &http.Response{
						StatusCode: 200,
			...
			stats, err := osa4.GetIndexStats(testCtx, "test-index")
			So(stats, ShouldBeNil)
			So(err, ShouldNotBeNil)
		})
	})
...
func Test_openSearchAccess_Refresh(t *testing.T) {
	Convey("test Refresh\n", t, func() {
		appSetting := &common.AppSetting{}
		osa, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
			roundTripFunc: func(req *http.Request) (*http.Response, error) {
				So(req.Method, ShouldEqual, "POST")
				So(req.URL.Path, ShouldContainSubstring, "test-index")
				So(req.URL.Path, ShouldContainSubstring, "_refresh")

				return &http.Response{
					StatusCode: 200,
					Body:       io.NopCloser(strings.NewReader(`{"_shards": {"total": 1, "successful": 1}}`)),
				}, nil
			},
		})

		Convey("Refresh Success \n", func() {
			err := osa.Refresh(testCtx, "test-index")
			So(err, ShouldBeNil)
		})

		Convey("Refresh Failed - HTTP error\n", func() {
			osa2, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
				roundTripFunc: func(req *http.Request) (*http.Response, error) {
					return nil, errors.New("network error")
				},
			...
			err := osa2.Refresh(testCtx, "test-index")
			So(err, ShouldNotBeNil)
		})

		Convey("Refresh Failed - response error\n", func() {
			osa3, _ := MockNewOpenSearchAccess(appSetting, &mockTransport{
				roundTripFunc: func(req *http.Request) (*http.Response, error) {
					return &http.Response{
						StatusCode: 400,
			...
			err := osa3.Refresh(testCtx, "test-index")
			So(err, ShouldNotBeNil)
		})
	})
}
