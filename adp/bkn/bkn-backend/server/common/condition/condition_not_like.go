...
func NewNotLikeCond(ctx context.Context, cfg *CondCfg, fieldsMap map[string]*FieldCfg) (Condition, error) {
	if !dtype.DataType_IsString(cfg.FieldCfg.Type) &&
		dtype.SimpleTypeMapping[cfg.FieldCfg.Type] != dtype.SimpleChar {
		return nil, fmt.Errorf("condition [not_like] left field is not a string field: %s:%s", cfg.FieldCfg.Name, cfg.FieldCfg.Type)
	}
