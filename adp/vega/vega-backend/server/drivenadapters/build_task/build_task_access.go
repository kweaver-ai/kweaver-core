...
func (bta *buildTaskAccess) GetAll(ctx context.Context, offset, limit int) ([]*interfaces.BuildTask, int64, error) {
	ctx, span := ar_trace.Tracer.Start(ctx, "Get all build tasks", trace.WithSpanKind(trace.SpanKindClient))
	defer span.End()

	// Get total count
	var totalCount int64
	countQuery := `SELECT COUNT(*) FROM ` + BUILD_TASK_TABLE_NAME
	err := bta.db.QueryRowContext(ctx, countQuery).Scan(&totalCount)
	if err != nil {
		logger.Errorf("Get build tasks count failed: %v", err)
		o11y.Error(ctx, fmt.Sprintf("Get build tasks count failed: %v", err))
		span.SetStatus(codes.Error, "Get build tasks count failed")
		return nil, 0, err
	}
	...
	rows, err := bta.db.QueryContext(ctx, query, limit, offset)
	if err != nil {
		logger.Errorf("Get all build tasks failed: %v", err)
		o11y.Error(ctx, fmt.Sprintf("Get all build tasks failed: %v", err))
