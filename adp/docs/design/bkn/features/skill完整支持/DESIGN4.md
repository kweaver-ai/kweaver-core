...
- target_rules 的语义不因 source 形态变化，由各 TargetRule 自身的 `scope/id/condition` 独立决定；`condition` 仅在 `scope=object_type` 时支持，表示对目标 ObjectType 实例行做字段条件收窄，其他 scope 不携带 condition

**RelationType 顶层字段约束（实现约束）：**

`scope_binding` 的 source / target 语义以 `mapping_rules.source` 与 `mapping_rules.target_rules` 为准；RelationType 存量模型中的顶层 `source_object_type_id` 作为 source 侧索引冗余字段，`target_object_type_id` 对 `scope_binding` 不适用。创建/更新时 BKN 会将 `source_object_type_id` 归一化为 `mapping_rules.source.object_type_id`，并始终将 `target_object_type_id` 置为空串，避免多个 target_rules 下顶层 target 字段语义不明确。
...
| **M4** | `scope_binding` mapping type 实现（仅支持 source=ObjectType(+filter)；source 业务 ObjectType 时支持 rid_property）；TargetRule 结构与校验；target 侧 fallback 语义 | M3 | scope_binding 可配置并完成 schema / 依赖校验；资源载体 source / 业务 source 两条路径的召回实现不在本轮 |
