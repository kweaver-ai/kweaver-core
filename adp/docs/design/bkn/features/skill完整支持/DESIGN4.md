...
`scope_binding` 的 source / target 语义完全由 `mapping_rules.source` 与 `mapping_rules.target_rules` 承载；RelationType 存量模型中的顶层 `source_object_type_id` / `target_object_type_id` 对 `scope_binding` 不适用。创建/更新时无论调用方是否传入这两个顶层字段，BKN 都会将其归一化为空串后落库，避免顶层字段与 mapping_rules 语义分裂。
