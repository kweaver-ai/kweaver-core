...
- **多模块通用**：框架与 ADP 各子仓库解耦；**VEGA 仅为调试/示例模块**。通过运行参数或环境变量 `CASE_FILE` / `AT_CASE_FILE` 指向 `testcase/<模块>`（如后续 `dataflow`、`ontology`）。会话清理等业务逻辑放在各模块的 `framework_hooks.py`，框架 `conftest` 仅负责调度。
...
项目根目录/
├── config/
│   └── config.ini              # 运行配置：host、base_url、application_token
├── testcase/
│   ├── _config/                # 框架级（所有模块共用）
│   │   ├── spec/
...
已在「快速开始」中说明；`CASE_FILE` / `AT_CASE_FILE` 决定当前运行哪个模块，`AT_CLEAN_UP_MODULE` 决定仅对哪个模块执行 session 级清理（若 conftest 中实现了清理逻辑）。
...
5. `config.ini`：`server.host`、`server.base_url`、`test_data.application_token` 与目标环境一致。
...
**步骤 2 — 按选出的 name 执行**（在项目根目录，且已设置 `CASE_FILE=./testcase/vega`）：
...
基于本仓库整理的**流程、数据流、目录结构与组件说明**，见 **[docs/测试框架文档.md](docs/测试框架文档.md)**。
