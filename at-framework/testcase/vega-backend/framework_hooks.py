...
def session_setup(config: Dict[str, Dict[str, str]], allure):
    """会话开始前的设置"""
    allure.attach("开始清理 vega-backend 历史数据", name="session_setup")
    ...
    if response.status_code != 200:
        allure.attach("清理 vega-backend 历史数据失败：%s" % response.content, name="session_setup")
        return
