...
class TestAgentDebug:
    ...
    @pytest.mark.api
    def test_agent_debug_conversation(self, collect_test_cases, AgentHeaders, AgentModelCheck):
        """
        测试智能体对话功能 - 数据驱动
            ...
            with allure.step(f"测试 {test_id}"):
                ...
                try:
                    ...
                    if query_type == "multi_turn" and agent_context_key in agent_conversation_context:
                        ...
                        debug_request_data["conversation_id"] = conversation_id
                    ...
                    if test_case.get("need_temp_file") and test_case.get("temp_file_config"):
                        debug_request_data["input"]["temp_files"] = test_case["temp_file_config"]
    ...
    @pytest.mark.api
    def test_agent_multi_turn_debug_conversation(self, AgentImport, AgentHeaders, AgentModelCheck):
        """
        测试智能体多轮对话功能
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            if available_agent.get("need_temp_file") and available_agent.get("temp_file_config"):
                debug_request_data["temp_files"] = available_agent["temp_file_config"]
            ...
            if is_intervention_test and i == 2:
                ...
                if intervention_info and first_response_assistant_message_id:
                    ...
                    debug_request_data["input"]["interrupted_assistant_message_id"] = first_response_assistant_message_id
            ...
            # 发起对话请求
            result = app_client.DebugChat(agent_id, debug_request_data, AgentHeaders, timeout=1200)
            ...
            if result[0] in [502, 503, 504] or (isinstance(result[1], str) and "Connection refused" in result[1]):
                ...
                if i == 1:
                    ...
                    if is_intervention_test and i == 2:
                        ...
                        # debug not support intervention
