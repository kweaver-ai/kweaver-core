...
from mem0.memory.storage import SQLiteManager, RDSManager
...
# 注释掉mem0自带 logger， 使用 src/utils/logger.py
...
class Memory(MemoryBase):
        ...
        if not infer:
            ...
            for message_dict in messages:
                if (
                    ...
                    logger.warn(f"Skipping invalid message format: {message_dict}")
                    continue
    ...
    def reset(self):
        ...
        # """
        # logger.warn("Resetting all memories")
        ...
        # else:
        #     logger.warn("Vector store does not support reset. Skipping.")
...
class AsyncMemory(MemoryBase):
        ...
        *,
        ...
        if agent_id is not None and memory_type == MemoryType.PROCEDURAL.value:
            results = await self._create_procedural_memory(
                messages, metadata=processed_metadata, prompt=prompt, llm=llm, context=context
            )
            return results
        ...
        logger.info("create vector strore task")
        vector_store_task = asyncio.create_task(
            self._add_to_vector_store(messages, processed_metadata, effective_filters, infer, context)
        )
        logger.info("create graph task")
    ...
    async def _add_to_vector_store(
        ...
        if not infer:
            ...
            for message_dict in messages:
                ...
                msg_content = message_dict["content"]
                msg_embeddings = await asyncio.to_thread(self.embedding_model.embed, msg_content, "add", context)
                mem_id = await self._create_memory(msg_content, msg_embeddings, per_msg_meta, context)
        ...
        logger.infof(
            "Extraced facts from messages, response: %s",
            response
            )
        try:
            response = remove_code_blocks(response)
        ...
        async def process_fact_for_search(new_mem_content):
            embeddings = await asyncio.to_thread(self.embedding_model.embed, new_mem_content, "add", context)
        ...
        logger.info(
            "Retrievaling existing memories by new memory content"
        )
        ...
        try:
            logger.info("Generating response for update memory...")
            response = await asyncio.to_thread(
                self.llm.generate_response,
                messages=[{"role": "user", "content": function_calling_prompt}],
                response_format={"type": "json_object"},
        ...
        try:
            memory_tasks = []
            logger.info("Creating memory tasks")
            for resp in new_memories_with_actions.get("memory", []):
                try:
                    action_text = resp.get("text")
                    if not action_text:
    ...
    async def get_all(
        ...
        *,
        ...
        memories_task = asyncio.create_task(self._get_all_from_vector_store(effective_filters, limit))
        
        if self.enable_graph:
            graph_task = asyncio.create_task(self._get_all_from_graph(effective_filters, limit))
            all_memories_result = await asyncio.gather(memories_task, graph_task)
        else:
            all_memories_result = await memories_task
        ...
        #     all_memories_result = await future_memories.result()
        #     graph_entities_result = await future_graph_entities.result() if future_graph_entities else None
    ...
    async def search(
        ...
        *,
        ...
        logger.info("Searching memory from vector store")
        vector_store_task = asyncio.create_task(self._search_vector_store(query, effective_filters, limit, threshold, context))
        ...
        logger.infof("Retrieved %d memories from vector store", len(original_memories))
        if self.enable_graph:
            return {"results": original_memories, "relations": graph_entities}
    ...
    async def _search_vector_store(self, query, filters, limit, threshold: Optional[float] = None, context: Optional[Dict[str, Any]] = None):
        logger.info("Embedding search query")
        embeddings = await asyncio.to_thread(self.embedding_model.embed, query, "search", context)
    ...
    async def update(self, memory_id, data, context=None):
        """
        ...
        Args:
            memory_id (str): ID of the memory to update.
            data (dict): Data to update the memory with.
            context (dict, optional): Context information like user_id and visitor_type.
        ...
        embeddings = await asyncio.to_thread(self.embedding_model.embed, data, "update", context)
        existing_embeddings = {data: embeddings}

        await self._update_memory(memory_id, data, existing_embeddings, context=context)
        return {"message": "Memory updated successfully!"}
    ...
    async def _create_memory(self, data, existing_embeddings, metadata=None, context=None):
        logger.debug(f"Creating memory with {data=}")
        if data in existing_embeddings:
            embeddings = existing_embeddings[data]
        else:
            embeddings = await asyncio.to_thread(self.embedding_model.embed, data, memory_action="add", context=context)
    ...
    async def _create_procedural_memory(self, messages, metadata=None, llm=None, prompt=None, context=None):
        """
        ...
        try:
            if llm is not None:
                ...
                procedural_memory = await asyncio.to_thread(self.llm.generate_response, messages=parsed_messages, context=context)
        except Exception as e:
            logger.error(f"Error generating procedural memory summary: {e}")
            raise
        ...
        metadata["memory_type"] = MemoryType.PROCEDURAL.value
        embeddings = await asyncio.to_thread(self.embedding_model.embed, procedural_memory, memory_action="add", context=context)
        memory_id = await self._create_memory(procedural_memory, {procedural_memory: embeddings}, metadata=metadata, context=context)
    ...
    async def _update_memory(self, memory_id, data, existing_embeddings, metadata=None, context=None):
        logger.info(f"Updating memory with {data=}")
