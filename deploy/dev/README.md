...
| 4 | `bash ./dev/mac.sh data-services install` | **Yes** before Core — installs MariaDB, Redis, Kafka, Zookeeper, OpenSearch into the cluster (mac defaults skip a second ingress; enables local-path PV when no StorageClass exists) |
| 5 | `bash ./dev/mac.sh kweaver-core download` | Optional (local chart cache; **minimum** by default) |
| 6 | `bash ./dev/mac.sh kweaver-core install` | **Yes** to deploy Core (**`--minimum` implied**; same as explicit `--minimum` or add `--full` to opt out) |
...
4. `bash ./dev/mac.sh data-services install` — **装 Core 前需要**：向集群部署 MariaDB / Redis / Kafka / Zookeeper / OpenSearch（mac 默认不再重复装 ingress；无 StorageClass 时会尝试 local-path）。  
5. `bash ./dev/mac.sh kweaver-core download` — **可选**，只下载 chart 到本机，不装集群。  
6. `bash ./dev/mac.sh kweaver-core install` — **往集群装 Core**（封装默认 **`--minimum`**；完整依赖加 **`--full`**；少交互可在命令前加 `-y`）。  
7. `bash ./dev/mac.sh onboard` — **可选**（可加 `-y` 跳过交互）。
