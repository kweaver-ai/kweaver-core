...
# Parse agentoperator command arguments
parse_agentoperator_args() {
    local action="$1"
    shift
...
# Initialize agentoperator database using common database initialization function
init_agentoperator_database() {
    local sql_dir="${SCRIPT_DIR}/scripts/sql/agentoperator"
    
    # Only initialize database if RDS is internal (MariaDB installed in cluster)
    if ! is_rds_internal; then
        warn_external_rds_sql_required "AgentOperator" "${sql_dir}"
        log_warn "Skipping automatic AgentOperator database initialization (external RDS)"
        return 0
    fi
    
    init_module_database "agentoperator" "${sql_dir}"
}
...
install_agentoperator() {
    ...
    if ! init_agentoperator_database; then
        log_error "Failed to initialize agentoperator database"
        return 1
    fi
    ...
    for release_name in "${AGENTOPERATOR_RELEASES[@]}"; do
        install_agentoperator_release "${release_name}" "${release_name}" "${namespace}" "${HELM_CHART_REPO_NAME}" "${HELM_CHART_VERSION}"
    done
    
    log_info "agentoperator services installation completed"
}

# Install a single agentoperator release
install_agentoperator_release() {
...
# Uninstall agentoperator services
uninstall_agentoperator() {
    log_info "Uninstalling agentoperator services..."
    ...
    for ((i=${#AGENTOPERATOR_RELEASES[@]}-1; i>=0; i--)); do
        local release_name="${AGENTOPERATOR_RELEASES[$i]}"
    ...
    log_info "agentoperator services uninstallation completed"
}

# Show agentoperator services status
show_agentoperator_status() {
    log_info "agentoperator services status:"
